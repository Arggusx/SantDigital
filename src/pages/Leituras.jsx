...
// Substituir os dias da semana pelo formato desejado
  ...
  return (
    <>
      <h1 
        id="title2" 
        className={`font-bold text-left ml-10 pt-5 sm:text-2xl px-1
    ...
    <div className="mt-20 ml-10 relative">
      ...
      {mostrarSalmo && (
        <div className="absolute left-0 mt-2 p-5 bg-white w-[90%] md:w-[60%] lg:w-[50%]">
          <h2 className="text-2xl font-bold mb-5">Responsório {leituras.salmo[0].referencia}</h2>
          <p className="text-lg font-semibold mb-5">Refrão: {leituras.salmo[0].refrao}</p>
      ...
      {mostrarEvangelho && (
        <div className="absolute left-0 mt-2 p-5 bg-white w-[90%] md:w-[60%] lg:w-[50%]">
          <p className="text-lg font-semibold mb-5">Evangelho {leituras.evangelho[0].referencia}</p>
          <h2 className="text-xl font-semibold mb-5">{leituras.evangelho[0].titulo}</h2>
          <p className="italic border-l-2 border-gray-300 pl-3 text-lg mb-5"
             dangerouslySetInnerHTML={{ __html: inserirQuebrasDeLinha(leituras.evangelho[0].texto, 15) }}></p>
          <p>- Palavra do Senhor</p>
          <p className='font-semibold'>- Graças a Deus</p>   
        </div>
      )}
    </div>
  </>
